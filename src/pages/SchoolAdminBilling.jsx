...
import { Users, CreditCard, CheckCircle2, AlertCircle, Loader2, ExternalLink, Shield, ArrowUpCircle, RefreshCw } from 'lucide-react';
...
export default function SchoolAdminBilling() {
  ...
  const { data: schoolData, refetch } = useQuery({
    ...
    queryFn: async () => { const schools = await base44.entities.School.filter({ id: schoolId }); return schools[0]; },
    enabled: !!schoolId,
  });
  ...
  useEffect(() => {
    ...
    if (params.get('success')) { setMessage({ type: 'success', text: 'Subscription activated successfully!' }); refetch(); }
    else if (params.get('canceled')) { setMessage({ type: 'info', text: 'Checkout was canceled — no changes were made.' }); }
  }, [refetch]);
  ...
  const handleManageBilling = async () => {
    ...
    } catch { setMessage({ type: 'error', text: 'Failed to open billing portal.' }); setLoading(false); }
  };
  ...
  const handleUpgradeCheckout = async (targetPlan) => {
    ...
    try {
      ...
      window.location.href = response.data.url;
    } catch { setMessage({ type: 'error', text: 'Failed to start checkout.' }); setLoading(false); }
  };
  ...
  return (
    <RoleGuard allowedRoles={['school_admin', 'super_admin', 'admin']}>
      <SchoolAdminPageShell
        ...
        actions={
          <Button size="sm" variant="outline" onClick={() => refetch()} className="gap-1.5 text-slate-500 h-8 text-xs">
      ...
      >
        <div className="space-y-4">
          ...
          {message && (
            <Alert className={message.type === 'success' ? 'border-emerald-200 bg-emerald-50' : message.type === 'error' ? 'border-red-200 bg-red-50' : 'border-blue-200 bg-blue-50'}>
                ...
                <div className="md:col-span-2 bg-white rounded-xl border border-slate-200 p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-y border-slate-100">
                    ...
                    {billingStatus === 'active' && schoolData?.subscription_current_period_end && (
                      <div>
                        <p className="text-xs text-slate-400 font-medium">{schoolData.subscription_cancel_at_period_end ? 'Cancels On' : 'Next Renewal'}</p>
              ...
              <div className="grid md:grid-cols-2 gap-5">
                  ...
                  {hasAnyWarning && (
                    <Alert className="border-amber-200 bg-amber-50">
                      ...
                      <AlertDescription className="text-amber-800">
                        <strong>Approaching plan limits.</strong> Consider upgrading before limits are reached.
                        {upgradePlans.length > 0 && <button onClick={() => setShowUpgrade(true)} className="ml-1 font-semibold underline cursor-pointer">View options →</button>}
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
                <div className="bg-white rounded-xl border border-slate-200 p-5">
                  ...
                  <div className="space-y-4 text-sm text-slate-600">
                    {[
                      { color: 'bg-amber-400', text: <><strong>User limit:</strong> New invitations cannot be sent.</> },
                      { color: 'bg-amber-400', text: <><strong>Class limit:</strong> New classes cannot be created.</> },
                      { color: 'bg-blue-400', text: <><strong>No data is lost</strong> when limits are reached.</> },
                      { color: 'bg-indigo-400', text: <><strong>Warnings appear</strong> at 80% and 95% of each limit.</> },
                    ].map((item, i) => (
              ...
              <TabsContent value="upgrade" className="mt-5">
                <div className="grid md:grid-cols-2 gap-5">
                  {upgradePlans.map(targetPlan => {
                    ...
                    const newFeatures = Object.entries(limits.features).filter(([k, v]) => v === true && !planLimits?.features[k]).map(([k]) => k.replace(/_/g, ' '));
                    ...
                    return (
                      <div key={targetPlan} className={`bg-white rounded-xl border-2 p-6 ${targetPlan === 'professional' ? 'border-indigo-300' : 'border-slate-200'}`}>
                        {targetPlan === 'professional' && <div className="flex justify-end mb-2"><Badge className="bg-indigo-100 text-indigo-700 border-0">Most Popular</Badge></div>}
                        ...
                        <div className="space-y-3 text-sm mb-5">
                          ...
                          {newModules.length > 0 && (
                            <div>
                              ...
                              {newModules.map(m => <div key={m} className="flex items-center gap-2 text-emerald-700"><CheckCircle2 className="w-3.5 h-3.5" /><span className="capitalize">{m.replace(/_/g, ' ')}</span></div>)}
                            </div>
                          )}
                          {newFeatures.length > 0 && (
                            <div>
                              <p className="font-semibold text-slate-700 mb-1.5">New Features</p>
                              {newFeatures.map(f => <div key={f} className="flex items-center gap-2 text-emerald-700"><CheckCircle2 className="w-3.5 h-3.5" /><span className="capitalize">{f}</span></div>)}
                            </div>
                          )}
                        </div>
                        <Button onClick={() => handleUpgradeCheckout(targetPlan)} disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 gap-1.5">
